...
def preprocessConvai2_(split, develop=False):
    def deal_persona_dialogue(your_persona, partner_persona, dialogues):
        ...
        turns = []
        for index in range(history_min_length, len(dialogues)):
    ...
    data = []
    dialogue = json.load(open(f"convai2/{split}_both_original_no_cands.txt.json"))
    ...
    for index, line in enumerate(lines):  
        ...
        if index != len(lines) - 1:
            ...
            # in the final
            instance = deal_persona_dialogue(speaker1, speaker2, dialogues)
            dia = {"id": dialogue_id, "dataset": "Convai2", "speaker1": speaker1, "speaker2": speaker2,
                    "utterances": instance["turns"]}
            dialogue_id += 1
            data.append(dia)
            if instance["utterances"]: 
                outputs[mode].append(instance)
...
def preprocessConvai2(develop=False):

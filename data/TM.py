            ...
            if(t["speaker"]=="USER"):
                ...
                if(len(turns)!=0 and turns[-1]['spk']=="SYSTEM"):
                    ...
                    if "segments" in t and type(turns[-2]["struct"])==list:
                        ...
                        turns[-2]["struct"] = t["segments"] # In API-OUT, struct is ""
                    else:
                        turns[-2]["struct"] = ""
                else:
...
def preprocessTM2019(develop=False):
    ...
    for dial in data:
        if(len(dial["services"])==1):
            # str(sorted(dial["services"])): let list to be str, so can be a key in dict
            data_by_domain[str(sorted(dial["services"]))].append(dial) # split the data by domain
            # print(f'str(sorted(dial["services"])): {str(sorted(dial["services"]))}')
            # exit(1)
    ...
    for dom, data in data_by_domain.items():
        ...
        # exit(1)
        data_by_domain_new[str(sorted([remove_numbers_from_string(s) for s in eval(dom)]))].append([train_data, dev_data, test_data]) # get train. vaild and test data by domain
